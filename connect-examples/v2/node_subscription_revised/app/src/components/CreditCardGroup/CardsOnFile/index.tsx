...
const CardsOnFile: React.FC<CardsOnFileProps> = ({ 
    ...
    const CardDropdown = ({cards}: any) => {
        ...
        return <form className="max-w-sm w-56 mx-auto mb-4">
            <label htmlFor="cards" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a card</label>
            <select 
                id="cards" 
                ...
                >
                {
                    cards.map((card: any, i: number) => {
                        return <option 
                        key={i}
                        value={card.id}>{card.cardBrand} ending in {card.last4}
                        </option>
                    })
                }
            </select>
