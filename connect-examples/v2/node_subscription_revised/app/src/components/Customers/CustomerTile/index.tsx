...
const CustomerTile: React.FC<CustomerTileProps> = ({ customer, selectedCustomerId, handleCustomerSelection }) => {
    ...
    return (
        <Card className={`max-w-sm mr-4 cursor-pointer ${isSelected ? 'bg-gray-100' : ''} ${handleCustomerSelection ? '' : 'cursor-default hover:bg-white'}`}
            onClick={handleCustomerSelection ? () => handleCustomerSelection(customer) : undefined}
        >
            ...
            <button
                type="button"
                className={`w-full rounded-lg bg-cyan-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-cyan-700 focus:outline-none focus:ring-4 focus:ring-cyan-200 dark:focus:ring-cyan-900 ${isSelected ? 'mt-4' : ''}`}
