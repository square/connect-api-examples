...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const {
      result: { objects },
    } = await squareClient.catalogApi.searchCatalogObjects({
...
router.post("/", async (req: Request, res: Response) => {
  ...
  try {
    const {
      result: { location },
    } = await squareClient.locationsApi.retrieveLocation("main");
    if (location?.id) {
  ...
  try {
    const {
      ...
      req.body.subscriptionVariationId,
    );
    if (
      object &&
      object.subscriptionPlanVariationData &&
      object.subscriptionPlanVariationData.phases
    )
...
router.post("/search", async (req: Request, res: Response) => {
  try {
    ...
    } = await squareClient.subscriptionsApi.searchSubscriptions({
      query: {
        filter: {
          customerIds: [req.body.customerId],
        },
      },
    });
    ...
    if (subscriptions?.length) {
      for (let i = 0; i < subscriptions?.length; i++) {
        ...
        if (invoiceId && subscriptions[i].id) {
          const {
            result: { invoice },
          } = await squareClient.invoicesApi.getInvoice(invoiceId);
...
router.post(
  "/:subscriptionId/actions/:actionId",
  async (req: Request, res: Response) => {
    try {
      const {
        result: { subscription },
      } = await squareClient.subscriptionsApi.deleteSubscriptionAction(
      ...
      console.error(
        `Error performing action on Square Subscription (Action ID: ${req.params.actionId}):`,
        error,
      );
...
router.post("/:subscriptionId/pause", async (req: Request, res: Response) => {
  try {
    const {
      result: { subscription },
    } = await squareClient.subscriptionsApi.pauseSubscription(
      req.params.subscriptionId,
      {
