...
function openModal(element){ // Changes hash which triggers which item's modal is on display
  showModal("modal-" + element.parentElement.id);
  window.history.pushState(null, null, `#${element.parentElement.id}`);
}
function showModal(id) {
  const modal = document.getElementById(id);
  if (modal){ // Only show modal if element exists
...
function closeModal() {
  ...
  if (modal){ // If open modal exist it closes it
...
function onUrlChange() { // function is called whenever there is a change to URL
  const itemId = location.hash.slice(1);
  if (itemId !== ""){ // If hash exists show modal
